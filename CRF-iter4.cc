...
namespace CRF
{
  ...
  */
  ...
  void CRFProblem<dim>::solve()
  {
      ...
      SolverControl solver_control_A(solution.block(0).size(),1.e-6);
      ...
      SolverControl solver_control_schur(P.size(), 1.e-12);
      SolverCG<Vector<double>> solver_schur(solver_control_schur);

      // FIXME: use a preconditioner
      ...
      // FIXME: currently just using direct solves for advection equations
      // Solver for advection equations
      SolverControl solver_control_advection(solution.block(2).size(), 1.e-6);
